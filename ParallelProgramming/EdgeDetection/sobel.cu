...
int main(int argc, char** argv) {
    ...
    unsigned *dev_height, *dev_width, *dev_channels;
    unsigned char *device_s, *device_t;

    cudaMalloc((void **)&dev_height, sizeof(unsigned));
    cudaMalloc((void **)&dev_width, sizeof(unsigned));
    cudaMalloc((void **)&dev_channels, sizeof(unsigned));
    cudaMemcpy(dev_height, &height, sizeof(unsigned), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_width, &width, sizeof(unsigned), cudaMemcpyHostToDevice);
    cudaMemcpy(dev_channels, &channels, sizeof(unsigned), cudaMemcpyHostToDevice);
    ...
    int num_blocks = 128;
    int block_size = std::ceil(height/(double)num_blocks);
    sobel<<<block_size, num_blocks>>>(device_s, device_t, height, width ,channels);
