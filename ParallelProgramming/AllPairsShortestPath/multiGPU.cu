...
void block_FW(){
    ...
    #pragma omp parallel num_threads(2)
    {
		...
		cudaMalloc((void**)&dev[threadId], matrixSize);

        /* Modified*/
        int roundPerThread = (threadId == 0) ?ceil(round, 2) :(round / 2);
        int yOffset = (threadId == 0) ?0 :ceil(round, 2);

        dim3 grid_dim(num_blocks, roundPerThread);

        cudaMemcpy(dev[threadId] + yOffset * ROW_SIZE, Dist + yOffset * ROW_SIZE, roundPerThread * ROW_SIZE * sizeof(int), cudaMemcpyHostToDevice);

        for (int r = 0; r < round; ++r){
            if(r >= yOffset && r < yOffset + roundPerThread){
                cudaMemcpy(Dist + r * ROW_SIZE, dev[threadId] + r * ROW_SIZE, ROW_SIZE * sizeof(int), cudaMemcpyDeviceToHost);
            }
            #pragma omp barrier

            if(r < yOffset || r >= yOffset + roundPerThread){
                cudaMemcpy(dev[threadId] + r * ROW_SIZE, Dist + r * ROW_SIZE, ROW_SIZE * sizeof(int), cudaMemcpyHostToDevice);
            }

            Phase1<<<1, block_dim>>>(dev[threadId], r, n);
            Phase2<<<num_blocks, block_dim>>>(dev[threadId], r, n);
            Phase3<<<grid_dim, block_dim>>>(dev[threadId], r, n, yOffset);
        }

        cudaMemcpy(Dist + yOffset * ROW_SIZE, dev[threadId] + yOffset * ROW_SIZE, roundPerThread * ROW_SIZE * sizeof(int), cudaMemcpyDeviceToHost);
        #pragma omp barrier
        /*End*/
    }
    cudaFree(dev);
...
__global__ void Phase3(int *dist, int Round, int n, int yOffset){
    ...
    int i = threadIdx.y;
    int j = threadIdx.x;
    int block_i = blockIdx.y + yOffset;
...
int main(int argc, char *argv[]){
    ...
    n = _n + (BLOCK_SIZE - (_n % BLOCK_SIZE)); // padding, let n % BLOCK_SIZE == 0
