...
int iters, width, height, curHeight;
...
int getHeight(){
    if(curHeight < height) return curHeight++;
    return -1;
}
...
int main(int argc, char** argv) {
	...
	for(int t = 0; t < ncpus; t++){
		  pthread_create(&threads[t], NULL, mandelbrot_set, NULL);
	}
  ...
  for(int t = 0; t < ncpus; t++){
    ...
    /* draw and cleanup */
    write_png(argv[1], iters, width, height, image);
    free(image);
}
