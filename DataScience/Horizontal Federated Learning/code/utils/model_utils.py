...
from FLAlgorithms.trainmodel.resnet import ResNet18

from torch.utils.data import DataLoader
# from FLAlgorithms.trainmodel.generator import Generator # 不需要Generator
...
def get_data_dir(dataset):
    ...
    elif 'cifar10' in dataset.lower(): # 照MNIST去改
        dataset_ = dataset.lower().replace('alpha', '').replace('ratio', '').replace('users', '').split('-')
        # print(dataset_)
        alpha, ratio, users=dataset_[1], dataset_[2], dataset_[3]
        #path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix=os.path.join('data', 'CIFAR10', 'u{}c10-alpha{}-ratio{}'.format(users, alpha, ratio))
        train_data_dir=os.path.join(path_prefix, 'train')
        test_data_dir=os.path.join(path_prefix, 'test')
        proxy_data_dir = 'data/proxy_data/CIFAR10-n10/'
...
def convert_data(X, y, device, dataset=''):
    if not isinstance(X, torch.Tensor):
        if 'celeb' in dataset.lower():
            X=torch.Tensor(X).type(torch.float32).permute(0, 3, 1, 2).to(device)
...
def read_user_data(index, data, device, dataset='', count_labels=False):
    #data contains: clients, groups, train_data, test_data, proxy_data(optional)
    id = data[0][index]
    train_data = data[2][id]
    test_data = data[3][id]
    X_train, y_train = convert_data(train_data['x'], train_data['y'], device=device, dataset=dataset)
    train_data = [(x, y) for x, y in zip(X_train, y_train)]
    X_test, y_test = convert_data(test_data['x'], test_data['y'], device=device, dataset=dataset)
...
def create_model(model, dataset, device):
    ...
    # num_class = 0
    # if passed_dataset == "cifar10": num_class = 10

    num_class = 10
    
    if model == "resnet18":
        model = ResNet18(num_class=num_class).to(device), model
    else:
        raise ValueError(f"{model} has not been implement")
        #model = Net(passed_dataset, model).to(device), model
