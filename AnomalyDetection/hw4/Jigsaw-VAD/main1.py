...
def get_configs():
    ...
    parser.add_argument("--dataset", type=str, default="avenue", choices=['shanghaitech', 'ped2', 'avenue'])
...
def train(args):
    ...
    # Load Data
    data_dir = f"/home/tunyuan/Jigsaw-VAD/dataset/{args.dataset}/training"
    ...
    for epoch in range(args.epochs):
        for it, data in enumerate(vad_dataloader):
            ...
            #print(temp_labels.shape, spat_labels.shape)
            #print(temp_logits.shape, spat_logits.shape)
            ...
            if (global_step + 1) % args.print_interval == 0:
                print("[{}:{}/{}]\tloss: {:.4f} t_loss: {:.4f} s_loss: {:.4f} \ttime: {:.4f}".\
            ...
            if global_step % args.val_step == 0 and epoch >= 5:
                ...
                if smoothed_auc > max_acc:
                    ...
                    save = './checkpoint/{}_{}.pth'.format('best', '1')
...
def val(args, net=None):
    ...
    # Load Data
    data_dir = f"/home/tunyuan/Jigsaw-VAD/dataset/{args.dataset}/testing"
...
def save_and_evaluate(video_output, running_date, dataset='avenue'):
...
if __name__ == '__main__':
    ...
    train(args)
    #net = train(args)
    #micro_auc, macro_auc, running_data = val(args, net)
    #print(micro_auc)